...
void Rules::initBoard(Board *board) {
    board->setPlayerAt(Rules::WHITE, 3, 3);
    board->setPlayerAt(Rules::BLACK, 3, 4);
    board->setPlayerAt(Rules::WHITE, 4, 4);
    board->setPlayerAt(Rules::BLACK, 4, 3);
...
bool Rules::isValidInput(string input, Board *board) {
    ...
    return isValidSpot(input) && isValidMove(board, input[1] - '1', input[0] - 'A');
}
...
void Rules::flipDiscs(Board *board, int x, int y, int offsetX, int offsetY) {
    do {
        ...
        if (board->getCurrentPlayer() == Rules::BLACK){ board->setBlackDiscCount(board->getBlackDiscCount()+1); board->setWhiteDiscCount(board->getWhiteDiscCount()-1);
        } else {
